...
async def message_handler_infobot(event: Message):
    ...
    try:
        await send_rich_message(event.chat_id, service.main_text(payload), buttons=buttons)
    except RichMessageError as rich_exc:
        ...
        await event.reply(msg, formatting_entities=entities, buttons=buttons)
